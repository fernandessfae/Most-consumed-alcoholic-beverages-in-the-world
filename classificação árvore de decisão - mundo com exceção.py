...
'''Aqui ulitizaremos o método de classificação com a árvore de decisão, utilizando validação cruzada
   e holdout, para ver se eles acertam a bebida marjoritária de cada país, exceto os países não alcoólicos. '''
...
#Criação de classificador(es) do tipo árvore de decisão
pip = Pipeline([('tree', DecisionTreeClassifier())])

#Verificação dos parâmetros que podem ser colocados no classificador
sorted(pip.get_params().keys())

#Criação de uma lista com diferentes parâmetros da arvore de decisão
min_samples_split = [2, 3, 4, 5, 6, 7]
min_samples_leaf = [2, 3, 4, 5, 6, 7]
criterion = ['entropy', 'gini']

#Dicionário para os parâmetro GRID
parametros_grid = dict(tree__min_samples_split = min_samples_split,
...
#Imprime os melhores parâmetros e acurácia da melhor árvore de decisão
grid.best_params_
grid.best_score_

#Criação do classificador
clf = DecisionTreeClassifier(criterion = 'entropy', min_samples_leaf = 2, min_samples_split = 5)
...
print(f'O desvio padrão da soma de todos os folds do modelo DecisionTreeClassifier é de {round(resultado.std(), 4)}')
print(f'A acurácia do modelo DecisionTreeClassifier é de {round(metrics.accuracy_score(classe,resultados) * 100, 2)}%')
print(f'As métricas do modelo DecisionTreeClassifier é:\n {metrics.classification_report(classe,resultados,valor_classes)}')  
...
export_graphviz(clf, out_file = 'tree_arvore_com_excecao.dot')
...
plt.title('Medição da acurácia do modelo - DecisionTreeClassifier', fontsize = 16, fontweight = 'bold')
